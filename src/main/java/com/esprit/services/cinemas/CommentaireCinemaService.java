...
public class CommentaireCinemaService implements IService<CommentaireCinema> {
    ...
    @Override
    public void create(CommentaireCinema cinemamcoment) {
        String req = "INSERT into commentairecinema(idclient,idcinema,commentaire,sentiment ) values (?,?,?,?);";
        try {
            PreparedStatement pst = connection.prepareStatement(req);
            pst.setInt(1, cinemamcoment.getClient().getId());
            pst.setInt(2, cinemamcoment.getCinema().getId_cinema());
            pst.setString(3, cinemamcoment.getCommentaire());
            pst.setString(4, cinemamcoment.getSentiment());
