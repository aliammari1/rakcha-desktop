 ...
 * database
 * application. It provides functions to read and update actor data, as well as
 * delete
 * actors from the database. Additionally, it provides an event handler for
 * switching
 * to a new stage with a different FXML layout when the "Ajouter Cinéma" button
 * is clicked.
 */
public class ActorController {
     ...
     *
     * @param searchText search query that filters the actors in the
     *                   `filteredActors` list.
     */
     ...
     *
     * @param event open file selection event that triggered the function to
     *              execute.
     *              <p>
     *              - Type: `ActionEvent` (represents an action event triggered by a
     *              user)
     *              - Target: null (indicates that the event is not associated with
     ...
     *
     * @param message content to be displayed in an alert window when the
     *                `showAlert()`
     *                method is called.
     */
    @FXML
     ...
     * object
     * with the actor's name, URI, and bio, and adds it to the ActorService using
     * the
     * create method. It then displays an information alert with the title "Actor
     * ajoutée"
     * and content text "Actor ajouté!" before refreshing the actor table.
     *
     * @param event user's action of clicking on the "Insert Actor" button, which
     *              triggers
     ...
     * is
     * committed, it updates the corresponding actor's property with the new value
     * from
     * the column edit.
     */
    private void setupCellOnEditCommit() {
        this.nomAcotr_tableColumn1.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Actor, String>>() {
             ...
             * to
             * the nominated value, and then updates the corresponding actor object with the
             * new
             * value.
             *
             * @param event CellEditEvent object that contains information about the edit
             *              event
             *              occurring on a table column, including the edited cell's
             *              position and the new value
             *              being entered.
             *
             *              - `TableColumn.CellEditEvent`: This is the type of event that
             *              triggered the
             *              function's execution.
             *              - `Actor`: This is the type parameter of the event, which
             *              represents the data
             *              type of the cell being edited.
             *              - `String`: This is the type of the value being edited in the
             *              cell.
             */
            @Override
            public void handle(final TableColumn.CellEditEvent<Actor, String> event) {
                try {
                    ...
                    ActorController.this.updateActor(event.getTableView().getItems().get(
        ...
        this.bioActor_tableColumn1.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Actor, String>>() {
             ...
             * for a
             * column containing a `String` type data. It updates the value of the
             * `biographie`
             * property of the corresponding actor object in the table view, and also
             * triggers
             * an update of the actor object itself.
             *
             * @param event CellEditEvent of a TableColumn, providing the edited cell and
             *              its new
             *              value.
             *
             ...
             *              - `Actor`: The data type of the cells being edited, which is an
             *              actor object in
             *              this case.
     ...
     *
     * @param event event of opening a file chooser, which triggers the code inside
     *              the
     ...
     * calling
     * the `update` method of the `ActorService` class and displaying an alert with
     * the
    ...
    void readActorTable() {
        try {
            ...
            this.nomAcotr_tableColumn1.setCellValueFactory((TableColumn.CellDataFeatures<Actor, String> actorStringCellDataFeatures) -> new SimpleStringProperty(actorStringCellDataFeatures.getValue().getNom())
                    /**
            ...
            this.DeleteActor_Column1.setCellValueFactory(filmcategoryButtonCellDataFeatures -> {
                     ...
                     */);
            this.bioActor_tableColumn1.setCellValueFactory((TableColumn.CellDataFeatures<Actor, String> actorStringCellDataFeatures) -> new SimpleStringProperty(actorStringCellDataFeatures.getValue().getBiographie()) /**
            ...
            this.imageAcotr_tableColumn1.setCellValueFactory((final TableColumn.CellDataFeatures<Actor, HBox> param) -> {
                    ...
                    hBox.addEventHandler(MouseEvent.MOUSE_CLICKED, (final MouseEvent event) -> {
                        try {
                            ...
                            if (null != file) {
                                final String destinationDirectory = "./src/main/resources/img/films/";
                                final Path destinationPath = Paths.get(destinationDirectory);
                                final String uniqueFileName = System.currentTimeMillis() + "_"
                                        + file.getName();
                                final Path destinationFilePath = destinationPath.resolve(uniqueFileName);
     ...
     *
     * @param event action event that triggered the function execution, providing
     *              the
    ...
    public void switchtoajouterCinema(final ActionEvent event) {
        try {
            final FXMLLoader fxmlLoader = new FXMLLoader(this.getClass().getResource("/InterfaceFilm.fxml"));
