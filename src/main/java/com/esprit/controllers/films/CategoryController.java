 ...
 * provides
 * a setupCellFactory method to set up cell factories for each column in the
 * table,
 * including id, nom, and description. The setupCellValueFactory method sets the
 * cell
 * value factory for each column, and the setupCellOnEditCommit method sets the
 * on
 * edit commit event handler for each column. These methods work together to
 * handle
...
public class CategoryController {
     ...
     * factories,
     * value factories, and on-edit commit listeners. It also reads data from a
     * category
     ...
     * category
     * name and description, then calls the `create()` method of a `CategoryService`
     * class
     ...
     * factories
     * create custom TableCells that display a text field with an error message when
     * the
    ...
    private void setupCellFactory() {
        ...
        final Callback<TableColumn<Category, String>, TableCell<Category, String>> stringCellFactory = new Callback<>() {
             ...
             *          value.
             *          - `TextField` and `Validator`: These are objects used within the
             *          `call` function
             *          to create a text field with a validator. The `TextField` object
             *          represents the
             *          text field itself, while the `Validator` object defines the
             *          validation rules for
             *          the input field.
             *          - `localToScreen()`: This method is used to convert the local
             *          coordinates of the
             *          text field to screen coordinates, allowing us to position the
             *          tooltip correctly.
             */
            @Override
            public TableCell<Category, String> call(final TableColumn<Category, String> param) {
                return new TextFieldTableCell<>(new DefaultStringConverter()) {
                     ...
                     * uppercase
                     * letter, and displays an error tooltip if any of these conditions are true. It
                     * also
                     * listens for changes to the text property of a TextField and displays the
                     * error
     ...
     * property
     * value factories that return the id, description, nom, and delete buttons for
     * each
    ...
    private void setupCellValueFactory() {
        ...
        this.nomCategory_tableColumn.setCellValueFactory(
                new Callback<TableColumn.CellDataFeatures<Category, String>, ObservableValue<String>>() {
                     ...
                     *          represents a simple
                     *          string value.
                     *          - The value of the string property is obtained by calling the
                     *          `getNom()` method
        ...
        this.delete_tableColumn.setCellValueFactory(
                new Callback<TableColumn.CellDataFeatures<Category, Button>, ObservableValue<Button>>() {
                     ...
                     *          component.
                     *          - The output is initialized with a new instance of `Button`, which
                     *          has a text
                     *          property of `"delete"` and an `onAction` property that is set to a
                     *          lambda function
    ...
    private void setupCellOnEditCommit() {
        this.nomCategory_tableColumn.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Category, String>>() {
             ...
             *              - `event.getNewValue()` returns the new value to be set in the
             *              cell.
             */
            @Override
