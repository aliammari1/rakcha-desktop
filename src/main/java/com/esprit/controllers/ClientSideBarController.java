 ...
 * views in a client-side application. It provides methods for switching to
 * various sections such as events, movies, products, series, cinema, logout,
 * and profile. The class also initializes the application's user data.
 */
public class ClientSideBarController implements Initializable {
    ...
    /**
     * Loads an FXML file named `AfficherProductClient.fxml`, and sets the scene of
    ...
    /**
     * Loads a new FXML file "/ui/series/SeriesClient.fxml" when the " serieButton"
     * action is triggered, replacing the current stage with a new scene containing
     ...
     * state by calling its superclass's `initialize` method and performing any
