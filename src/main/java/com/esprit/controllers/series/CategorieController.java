 ...
 * between
 * different views of the application, such as the main menu, series view,
 * episode
...
public class CategorieController {
     ...
     * adding
     * new columns for an "Edit" and "Delete" button, and finally adding the
     * recovered
    ...
    private void ref() {
        ...
        final TableColumn<Categorie, String> nomCol = new TableColumn<>("Name");
        nomCol.setCellValueFactory(new PropertyValueFactory<>("nom"));
        final TableColumn<Categorie, String> descriptionCol = new TableColumn<>("Description");
        descriptionCol.setCellValueFactory(new PropertyValueFactory<>("description"));
        final TableColumn<Categorie, Void> supprimerCol = new TableColumn<>("Delete");
        supprimerCol.setCellFactory(param -> new TableCell<>() {
            private final Button button = new Button("Delete");

            {
                this.button.setOnAction(event -> {
                    final Categorie categorie = this.getTableView().getItems().get(this.getIndex());
                    try {
                        categorieserv.supprimer(categorie.getIdcategorie());
                        CategorieController.this.tableView.getItems().remove(categorie);
                        ...
                        CategorieController.this.showAlert("Error", e.getSQLState());
                    }
                });
            }
             ...
             *              be updated.
             *
             * @param empty whether the item is empty or not, which determines whether the
             *              graphic
        ...
        final TableColumn<Categorie, Void> modifierCol = new TableColumn<>("Edit");
        modifierCol.setCellFactory(param -> new TableCell<>() {
...

            {
                this.button.setOnAction(event -> {
                    ...
                    if (2 == clickCount) {
                        final Categorie categorie = this.getTableView().getItems().get(this.getIndex());
                        CategorieController.this.modifierCategorie(categorie);
                        this.clickCount = 0;
                    }
                });
            }
             ...
             * to a button.
             *
             * @param item  Void object being updated, and its value is passed to the super
             *              method
             ...
             *              not.
             *
             * @param empty whether the item is empty or not, and determines whether the
             *              graphic
        ...
        try {
            this.tableView.getItems().addAll(categorieserv.recuperer());
        } catch (final SQLException e) {
            CategorieController.LOGGER.log(Level.SEVERE, e.getMessage(), e);
        }
    }
     ...
     *
     * @param title   title of an alert window that will be displayed when the
     *                `showAlert`
     *                method is called.
     ...
     * dialogue
     * box interface. The function calls the `iServiceCategorie.modifier()` method
     * to
     * update the category in the database after the user has entered the new
     * details and
     * confirmed the modifications.
     *
     * @param categorie category object that is to be modified through the dialog
     *                  box,
     *                  and its properties (name and description) are editable and
     ...
     */
    private void modifierCategorie(final Categorie categorie) {
        final IServiceCategorieImpl iServiceCategorie = new IServiceCategorieImpl();
        final Dialog<Pair<String, String>> dialog = new Dialog<>();
        dialog.setTitle("Edit Category");
        final TextField nomFild = new TextField(categorie.getNom());
     ...
     * `descreptionF.getText()`
     * field, and returns `true` if so, or `false` otherwise, along with a message
     * to
     ...
     * category
     * to a service implementation class, and displaying an alert message upon
     * successful
     * save.
     *
     * @param event user's action of clicking the "Add Category" button, which
     *              triggers
     *              the execution of the code within the function.
     *              <p>
     *              - Type: `ActionEvent`
     *              - Details: Contains information about the action that triggered
     *              the function,
    ...
    void ajouteroeuvre(final ActionEvent event) {
        ...
        if (this.checkname() && this.checkdescreption()) {
            try {
                categorie.setNom(this.nomF.getText());
     ...
     *              class, which
     *              provides no additional information beyond the fact that it is an
     *              event object.
     */
    @FXML
     ...
     *
     * @param event source of the action that triggered the method, providing a
     *              reference
     ...
     * triggered.
     *
     * @param actionEvent event that triggered the function, which is used to
     *                    determine
     ...
     *
     * @param actionEvent event that triggered the `showseries()` function to be
     *                    called,
