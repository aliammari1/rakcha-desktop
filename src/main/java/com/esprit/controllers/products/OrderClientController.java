 ...
 * modules. It provides an interface for loading new fxml files based on user
 * actions, such as showing a movie or an event. The controller also handles
 * closing the current stage and displaying a new one. Additionally, it includes
 * methods for handling Movie and Serie client interactions.
 */
public class OrderClientController implements Initializable {
    ...
    /**
     * Sets the value of a `Order` object and runs a lambda action to retrieve and
     * print the user's name, then creates a label with the total price and adds it
     * to a `FlowPane`.
     *
     * @param orderselectionner
     *            selected command that triggers the function to execute, which is
     *            used to retrieve the total price from SharedData and create a
     *            label displaying the result.
     *            <p>
     *            `orderselectionner`: An instance of `Order`, representing the
     *            selected command.
     */
    @FXML
    void initialize(final Order orderselectionner) {
        ...
        Platform.runLater(new Runnable() {
             ...
             */
            public void run() {
                OrderClientController.this.connectedUser = (Client) OrderClientController.this.prixtotaleFlowPane
                        .getScene().getWindow().getUserData();
                OrderClientController.LOGGER.info("produits:    user:  " + OrderClientController.this.connectedUser);
            }
        });
     ...
     *
     * @param url
     *            URL of the web application being initialized by the `initialize()`
     *            method.
     * @param resourceBundle
     *            collection of translated keys and their corresponding values
     */
    @Override
    /**
     ...
     * various styling properties to display the number in a specific format.
     *
     * @param prixTotal
     *            total price of an item, which is used to create a label with the
     *            value displayed.
     * @returns a label displaying the price total in bold font with a specific
     *          color.
     *          <p>
     *          - The output is a `Label` object named `prixTotalLabel`. - The
     *          label's text value is the sum of `prixTotal` and "DT". - The font of
     *          the label is set to Verdana with a size of 25. - The text fill color
     *          of the label is set to #d72222.
     */
     ...
     * method to create the command in the database. It then iterates through each
     * item in the command and creates a new item object, updates the inventory of
     * the product, and sets the payment line visible.
     *
     * @param event
     *            action event triggered by clicking on the "Order" button, which
     *            initiates the processing of the command and calls the various
     *            methods to create and update objects and database records.
     *            <p>
    ...
    void order(final ActionEvent event) {
        ...
        if (!this.isValidPhoneNumber(numTelephone)) {
            ...
            return; // Arrêter le traitement de la order
        }
        ...
        try {
            idorder = this.orderService.createOrder(this.order);
            this.order.setId(idorder);
        } catch (final SQLException e) {
            throw new RuntimeException(e);
        }
        for (final OrderItem orderItem : this.order.getOrderItems()) {
            OrderClientController.LOGGER.info(String.valueOf(this.order.getId()));
     ...
     * using the `showAndWait()` method.
     *
     * @param title
     *            title of an alert message displayed by the `showAlert()` method.
     * @param content
     *            text to be displayed in the alert window.
     */
     ...
     * digits and only containing numbers.
     *
     * @param phoneNumber
     *            8-digit phone number to be validated for its correct format,
     ...
     * specified quantity from the product's current quantity, and updates the
     * product record in the database using an instance of `ProductService`.
     *
     * @param produit
     *            product object whose stock is being decremented.
     *            <p>
     *            - `produit`: A `Product` object representing a product. -
     *            `quantity`: An integer representing the quantity of the product to
     *            be decremented.
     * @param quantity
     *            amount of stock to be decremented from the product's current
     ...
     * and replaces the current stage with the new one, closing the original stage.
     *
     * @param event
     *            MouseEvent object that triggered the function, providing
     *            information about the mouse button pressed, the screen position of
     *            the click, and other details.
     *            <p>
     *            - Type: `MouseEvent` indicating that the event was triggered by a
     *            mouse action. - Source: The object that generated the event, which
     *            is typically a button or link in the user interface. - Event type:
     *            The specific type of event, such as `MOUSE_CLICKED` or
     *            `MOUSE_OVER`.
     */
    @FXML
     ...
     * approval. It retrieves the payment details and redirects the user to the
     * appropriate URL based on the payment method.
     *
     * @param event
     *            event that triggered the `payment()` method and provides the
     *            necessary context for the payment creation process.
     *            <p>
        ...
        try {
            ...
            for (final Links link : createdPayment.getLinks()) {
                if ("approval_url".equalsIgnoreCase(link.getRel())) {
                    OrderClientController.LOGGER.info(link.getHref());
     ...
     * payment ID and payer ID as query parameters.
     *
     * @param approvalLink
     *            URL of the PayPal approval page, which is loaded into a WebView
     *            component and monitored for the successful redirection to a
     *            predefined URL indicating the completion of the payment process.
     */
    private void redirectToPayPal(final String approvalLink) {
        Platform.runLater(() -> {
            ...
            closeButton.setOnAction(e -> {
                OrderClientController.LOGGER.info("Close button clicked");
                dialog.hide();
            });
     ...
     * parameter name if it exists, or `null` otherwise.
     *
     * @param url
     *            URL to be analyzed and is used to extract the query parameter
     *            value using `URLEncodedUtils.parse()`.
     * @param parameterName
     *            name of the query parameter to be extracted from the URL.
     ...
     * state and updating the order status accordingly.
     *
     * @param paymentId
     *            unique identifier of the payment to be executed.
     * @param payerId
     *            identifier of the payer in the PayPal payment execution, which is
     ...
     *
     * @param event
     *            ActionEvent that triggered the function execution and provides
     *            access to information about the event, such as the source of the
     *            event.
     *            <p>
     *            - It is an `ActionEvent`, which means it carries information about
     *            the action that triggered the function. - The source of the event
     *            is a `Node`, which represents the element in the user interface
     ...
     * scene, then attaches it to an existing stage and replaces the current stage
     * with the new one, closing the original stage afterward.
     *
     * @param event
     ...
     *            event was triggered by a user action (e.g., clicking a button).
     */
    @FXML
     ...
     * and attaches it to a new stage. It then closes the current stage to display
     * the new one.
     *
     * @param event
     *            ActionEvent object that triggered the function execution,
     *            providing the source of the event and allowing for handling the
     *            corresponding action.
     *            <p>
     *            - `event`: an ActionEvent object representing a user action (e.g.,
     *            mouse click or key press) that triggered the function execution.
     */
    @FXML
     ...
     * tools to gather performance data on various aspects of the application, such
     * as CPU usage, memory allocation, and method execution time.
     *
     * @param event
     *            occurrence of an action event that triggered the function
     *            execution.
     */
    @FXML
     ...
     *
     * @param event
     *            occurrence of a button click event that triggered the
     *            `showcinema()` method invocation.
     */
    @FXML
     ...
     *
     * @param event
     *            AnimationEvent object that triggered the execution of the
     *            `showEvenement` method.
     */
    @FXML
     ...
     *
     * @param event
     *            ActionEvent object that triggered the showmovie method, providing
     *            information about the source of the event and allowing the method
     *            to handle the appropriate action accordingly.
     *            <p>
     *            - It is an instance of `ActionEvent`, which represents an action
     *            event occurred in the JavaFX application. - The source of the
     *            event is a `Node`, which can be any element in the FXML document,
     *            such as a button or a label.
     */
    @FXML
     ...
     *
     * @param event
     *            occurrence of an action, triggered by the user, that calls the
     *            `showproduit()` method.
     */
    @FXML
     ...
     *
     * @param event
     *            ActionEvent that triggered the function, providing the source of
     *            the event and allowing for proper handling of the action taken.
     *            <p>
     *            - `event` represents an ActionEvent object containing information
     *            about the action that triggered the function's execution.
     */
    @FXML
     ...
     * scene to a new stage. It also closes the current stage.
     *
     * @param event
     *            event that triggers the execution of the `MovieClient` method,
     *            which is an ActionEvent object containing information about the
     *            action that triggered the method call.
     *            <p>
     *            - `event` is an `ActionEvent` object representing the user's
     *            action that triggered the function. - The source of the event is a
     *            `Node` object, which provides information about the element that
     *            was clicked or selected.
     */
    @FXML
     ...
     * stage, while closing the current stage.
     *
     * @param event
     *            ActionEvent object that triggered the `SerieClient()` method to be
     *            called, providing the source of the event and any related data.
     *            <p>
     *            - It is an instance of `ActionEvent`, representing an action
     *            performed on the JavaFX application. - Its source field references
     *            the object that triggered the event, which in this case is a
     *            `Node` (i.e., a FXML component).
     */
    @FXML
