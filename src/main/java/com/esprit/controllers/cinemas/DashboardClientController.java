...
public class DashboardClientController {
    ...
    private HBox createCinemaCard(final Cinema cinema) {
        ...
        try {
            final String logoString = cinema.getLogo();
        ...
        logoImageView.setOnMouseClicked(event -> this.geocodeAddress(cinema.getAdresse()));
        ...
        final RatingCinemaService ratingService = new RatingCinemaService();
        final Rating rating = new Rating();
        rating.setLayoutX(100);
        rating.setLayoutY(100);
        rating.setMax(5);
        // Obtenez le taux spécifique pour ce client et ce cinéma
        final int specificRating = ratingService.getRatingForClientAndCinema(2, cinema.getId_cinema());
        ...
        rating.ratingProperty().addListener((observable, oldValue, newValue) -> {
            ...
            final RatingCinema ratingCinema = new RatingCinema(cinema, client, newValue.intValue());
    ...
    public void createTopRatedCinemaCards(final AnchorPane Anchortop3) {
        final RatingCinemaService ratingCinemaService = new RatingCinemaService();
        ...
        for (final Cinema cinema : topRatedCinemas) {
            ...
            card.getChildren().add(logoImageView);
            final Label nomLabel = new Label("Nom: " + cinema.getNom());
            nomLabel.setLayoutX(200);
            nomLabel.setLayoutY(10);
            card.getChildren().add(nomLabel);
            final Label adresseLabel = new Label("Adresse: " + cinema.getAdresse());
    ...
    private void geocodeAddress(final String address) {
        new Thread(() -> {
            try {
                final String apiUrl = "https://nominatim.openstreetmap.org/search?format=json&q="
                        + address.replaceAll(" ", "+");
    ...
    public void showPlanning(final Cinema cinema) {
        ...
        for (int i = 0; 7 > i; i++) {
            ...
            dayLabel.setOnMouseClicked(event -> this.displaySeancesForDate(date, cinema));
     ...
     */
    private Map<LocalDate, List<Seance>> loadCurrentWeekPlanning(final LocalDate startDate, final Cinema cinema) {
     ...
     *               and address.
     */
    private void displaySeancesForDate(final LocalDate date, final Cinema cinema) {
        // Charger les séances pour la date spécifiée
        final Map<LocalDate, List<Seance>> weekSeancesMap = this.loadCurrentWeekPlanning(date, cinema);
        final List<Seance> seancesForDate = weekSeancesMap.getOrDefault(date, Collections.emptyList());
        ...
        for (int i = 0; 7 > i; i++) {
            ...
            dayLabel.setStyle("-fx-background-color: #ae2d3c; -fx-padding: 10px; -fx-text-fill: white;");
            dayLabel.setOnMouseClicked(event -> this.displaySeancesForDate(currentDate, cinema));
            dayLabel.setFont(Font.font("Arial Rounded MT Bold", FontWeight.BOLD, 14));
        ...
        if (!seancesForDate.isEmpty()) {
            ...
            // sélectionnée
            final VBox seancesForDateVBox = new VBox();
            seancesForDateVBox.setSpacing(10);
            // Ajouter les cartes de séance au VBox
            for (final Seance seance : seancesForDate) {
                final StackPane seanceCard = this.createSeanceCard(seance);
            ...
            // spécifiée
            final Label noSeanceLabel = new Label("Aucune séance prévue pour cette journée.");
     ...
     */
    private StackPane createSeanceCard(final Seance seance) {
        ...
        try {
            final String logoString = seance.getFilmcinema().getId_film().getImage();
            final Image logoImage = new Image(logoString);
        ...
        final Label filmNameLabel = new Label("Film: " + seance.getFilmcinema().getId_film().getNom());
        filmNameLabel.setStyle("-fx-font-family: 'Arial Rounded MT Bold'; -fx-font-size: 16px;");
        final Label salleNameLabel = new Label("Salle: " + seance.getNom_salle());
        salleNameLabel.setStyle("-fx-font-family: 'Arial Rounded MT Bold'; -fx-font-size: 16px;");
        final Label timeLabel = new Label("Heure: " + seance.getHD() + " - " + seance.getHF());
        timeLabel.setStyle("-fx-font-family: 'Arial Rounded MT Bold'; -fx-font-size: 16px;");
        final Label priceLabel = new Label("Prix: " + seance.getPrix());
    ...
    public void initialize() {
        final CinemaService cinemaService = new CinemaService();
        this.l1 = cinemaService.read();
        this.searchbar1.textProperty().addListener((observable, oldValue, newValue) -> {
            final List<Cinema> produitsRecherches = DashboardClientController.rechercher(this.l1, newValue);
        ...
        this.listCinemaClient.setVisible(true);
        this.PlanningPane.setVisible(false);
        this.tricomboBox.getItems().addAll("nom");
        this.tricomboBox.setValue("");
        this.tricomboBox.getSelectionModel().selectedItemProperty().addListener((observableValue, s, t1) -> {
            this.cinemaFlowPane.getChildren().clear();
            final List<Cinema> filmList = new CinemaService().sort(t1);
            for (final Cinema film : filmList) {
                this.cinemaFlowPane.getChildren().add(this.createCinemaCard(film));
            }
        });
        this.createTopRatedCinemaCards(this.Anchortop3);
    }
     ...
     *                film schedule.
     */
    private void createfilmCards(final List<Cinema> Cinemas) {
    ...
    void filtrercinema(final ActionEvent event) {
        ...
        final List<Cinema> filteredCinemas = this.l1.stream()
    ...
    private List<String> getSelectedNames() {
        ...
        return this.namesCheckBoxes.stream()
                .filter(CheckBox::isSelected)
                .map(CheckBox::getText)
                .collect(Collectors.toList());
    }
     ...
     */
    @FXML
    void afficherProduitsClient(final ActionEvent event) throws IOException {
    ...
    void addCommentaire() {
        ...
        if (message.isEmpty()) {
            ...
            DashboardClientController.LOGGER.info(this.cinemaId + " " + new CinemaService().getCinema(this.cinemaId));
            final CommentaireCinema commentaire = new CommentaireCinema(new CinemaService().getCinema(this.cinemaId),
                    (Client) new UserService().getUserById(2), message, sentimentResult);
            DashboardClientController.LOGGER.info(commentaire + " " + new UserService().getUserById(2));
            final CommentaireCinemaService commentaireCinemaService = new CommentaireCinemaService();
     ...
     * class.
     */
    private List<CommentaireCinema> getAllComment(final int cinemaId) {
        final CommentaireCinemaService commentaireCinemaService = new CommentaireCinemaService();
        final List<CommentaireCinema> allComments = commentaireCinemaService.read();
        final List<CommentaireCinema> cinemaComments = new ArrayList<>();
        for (final CommentaireCinema comment : allComments) {
     ...
     */
    private HBox addCommentToView(final CommentaireCinema commentaire) {
        ...
        final Text userName = new Text(commentaire.getClient().getFirstName() + " " + commentaire.getClient().getLastName());
     ...
     *                 displayed.
     */
    private void displayAllComments(final int cinemaId) {
        final List<CommentaireCinema> comments = this.getAllComment(cinemaId);
        final VBox allCommentsContainer = new VBox();
        for (final CommentaireCinema comment : comments) {
