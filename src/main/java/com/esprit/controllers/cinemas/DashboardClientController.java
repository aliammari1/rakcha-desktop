...
public class DashboardClientController {
    ...
    /**
         ...
         * @param recherche the substring to match against each cinema's name; cinemas with a null name are ignored and matching is case-sensitive
         * @return a List<Cinema> containing cinemas from {@code liste} whose name contains {@code recherche}
         */
    @FXML
    /**
    ...
    /**
     * Load cinemas with status "Accepted" and display each as a UI card in the controller's cinemaFlowPane.
     *
    ...
    /**
     * Obtain geographic coordinates for the given address and display a map dialog centered at that location.
     *
    ...
    /**
     * Display the weekly planning and show the movie sessions for the specified date and cinema in the planning view.
     *
     * Updates the controller's planningFlowPane with a week header and either the list of sessions for the given date
     * or a message indicating no sessions are scheduled.
     *
     * @param date   the date whose sessions should be shown (a specific day within the displayed week)
    ...
    /**
     * Creates a UI card for each cinema and appends it to this controller's cinemaFlowPane.
     *
     * @param cinemas the cinemas for which cards will be created and added to the pane
     */
    ...
    /**
     * Show the filter pane and populate it with checkboxes for cinema addresses and names.
     *
     * Populates the filterAnchor with two VBoxes containing a labeled list of CheckBox nodes
     * for each unique cinema address and name, and visually dims the main cinema list.
     *
    ...
    /**
     * Apply the selected address and name filters to the cinema list and update the displayed cinema cards.
     *
     * Reads the selected addresses and names, filters the set of all cinemas so that cinemas match any selected
     * address and any selected name (an empty selection for a criterion means that criterion is not applied),
     ...
     *
     * @return a list of address strings for each selected CheckBox; empty if none are selected.
     */
     ...
     *
     * @return a list of selected checkbox texts representing names, or an empty list if none are selected.
     */
     ...
     *
     * <p>Loads the FXML resource "/ui/events/AffichageEvenementClient.fxml", shows it
     * in a new Stage titled "Event Client", and closes the stage that dispatched
     * the provided event.
     *
     * @param event the ActionEvent whose source window will be closed after the new window is shown
     * @throws IOException if the FXML resource cannot be loaded
     */
    @FXML
    void afficherEventsClient(final ActionEvent event) throws IOException {
        final FXMLLoader loader = new FXMLLoader(this.getClass().getResource("/ui/events/AffichageEvenementClient.fxml"));
    ...
    /**
     * Open the product client interface in a new window and close the current stage.
     *
     * @throws IOException if the FXML resource cannot be loaded
     */
    @FXML
     ...
     *
     * @throws IOException if the FXML resource "/ui/series/SeriesClient.fxml" cannot be loaded
     */
    @FXML
    ...
    /**
         * Add the text from the comment input as a cinema comment after performing sentiment analysis.
         *
         * <p>If the input is empty, a warning dialog is shown and no comment is added.
         * Otherwise the comment's sentiment is determined, a CinemaComment for the
         * currently selected cinema is persisted, and the input area is cleared.</p>
         */
    @FXML
    void addCommentaire() {
    ...
    /**
     * Save the current comment for the selected cinema and refresh the displayed comments.
     *
     * This handler is invoked when the Add Comment UI element is clicked; it stores the
     * comment entered by the user for the active cinema and reloads the comment view.
     */
    @FXML
    ...
    /**
     * Create an HBox view representing a cinema comment with the user's avatar, name, and message.
     *
     * If the comment's client has no profile image URL, a bundled default image is used.
     *
     * @param commentaire CinemaComment whose client (name and photo) and commentText supply the avatar, username, and message to display
     * @return an HBox containing the user's circular avatar and a text card with the user's name and comment, ready to be placed into the comments ScrollPane
     */
