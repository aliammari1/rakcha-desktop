 ...
 * service,
 * generates sentiment statistics for each cinema using the comments, and
 * creates
 * PieCharts to visualize the statistics. The controller also displays the
 * generated
 * PieCharts in an AnchorPane.
 */
public class CinemaStatisticsController {
    private final CommentaireCinemaService commentaireCinemaService = new CommentaireCinemaService();
     ...
     * generates
     * sentiment statistics for each cinema, and displays the statistics as
     * PieCharts
     * within a new container added to the parent AnchorPane.
     *
     * @param event ActionEvent triggered by the button click that initiates the
     *              code
    ...
    void showStatistics(final ActionEvent event) {
        ...
        final List<CommentaireCinema> comments = this.commentaireCinemaService.read();
     ...
     * comments,
     * where each comment is associated with a cinema and a sentiment label. The
     * function
     * counts the number of comments with each sentiment and stores it in the map
     * for
     * that cinema.
     *
     ...
     *                 <p>
     *                 - It is a list of `CommentaireCinema` objects, representing
     *                 user comments on
     *                 cinema movies.
     ...
     * - Each cinema name in the outer map corresponds to a map of
     * sentiment names in
     * the inner map, indicating the number of comments expressing that
     * sentiment for
     * that cinema.
     * - The maps are keyed by sentiment name and contain integer values
     * representing
     * the count of comments expressing that sentiment for each cinema.
     */
    private Map<String, Map<String, Integer>> generateSentimentStatistics(final List<CommentaireCinema> comments) {
        ...
        for (final CommentaireCinema comment : comments) {
            final String cinemaName = this.cinemaService.getCinema(comment.getCinema().getId_cinema()).getNom();
     ...
     *
     * @param cinemaName          name of the cinema for which sentiment statistics
     *                            are being
     *                            generated and displayed in the PieChart.
     * @param sentimentStatistics sentiment statistics of a cinema, which is used to
     *                            generate a pie chart displaying the distribution
     *                            of sentiment scores for that cinema.
     *                            <p>
     *                            - `Map<String, Integer>`: This map contains
     *                            sentiment statistics for different
     *                            cinema names, where each key represents a cinema
     *                            name and the value represents the
     ...
     * Statistics".
     * 2/ Data: The function adds the sentiment statistics data to the
     * PieChart's data
     * list. Each entry in the data list consists of a category (key) and
     * the corresponding
     * frequency (value).
     */
