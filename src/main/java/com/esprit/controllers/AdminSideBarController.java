 ...
 * administrator. It provides buttons to switch between views such as users,
 * events, movies, products, series, cinema, and profile. The controller also
 * includes a web view and handles the login and logout functionality.
 */
public class AdminSideBarController implements Initializable {
     ...
     * the application. It performs no additional actions and does not modify any
     * variables.
     *
     * @param event
    ...
    /**
     * Loads an fxml file named `ListeOrder.fxml` and sets the stage scene to
    ...
    void switchToMovies(final ActionEvent event) {
        try {
            ...
            // FilmController moviesessionController = loader.getController();
            // moviesessionController.setData(admin);
    ...
    void switchToProducts(final ActionEvent event) {
        try {
            ...
            // DesignProductAdminContoller moviesessionController = loader.getController();
            // moviesessionController.setData(admin);
            final Parent root = loader.load();
    ...
    /**
     * Loads a FXML file named "/ui/series/Categorie-view.fxml" into a Stage using
     * the `FXMLLoader`. It sets the data for the controller and then sets the scene
    ...
    void switchToSeries(final ActionEvent event) {
        try {
            ...
            // SerieClientController moviesessionController = loader.getController();
            // moviesessionController.setData(admin);
            final Parent root = loader.load();
    ...
    void switchtcinema(final ActionEvent event) {
        try {
            final FXMLLoader loader = new FXMLLoader(
                    this.getClass().getResource("/ui/cinemas/DashboardAdminCinema.fxml"));
            final Parent root = loader.load();
    ...
    @FXML
    /**
     * Performs switchToUsers operation.
     *
